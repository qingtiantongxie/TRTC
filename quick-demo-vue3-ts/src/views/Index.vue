  ...
  <el-row style='padding: 0 10px 40px 10px'>
    <el-col :md='{span: 18, offset: 3}' :sm='{span: 24}'>
      <div class='btn-line'>
        ...
        <el-button type='primary' @click='handleUnpublish'>不发布（远端用户将不能访问您）</el-button>
        <el-button type='primary' @click="copy">分享给好友</el-button>
        <el-button type='primary' @click='handleStartShare'>开启屏幕共享</el-button>
        <el-button type='primary' @click='handleStopShare'>关闭屏幕共享</el-button>
        <el-button type='primary' @click='handleLeave'>离开</el-button>
      </div>
      <div class='pusher'>
...
function copy() {
  copyText(inviteLink.value, undefined, (error: any) => {
    if (error) {
      ...
      ElMessage({ message: 'Copied!去分享链接给微信好友', type: 'success' });
    }
    inviteLink.value = store.createShareLink();
  });
}
